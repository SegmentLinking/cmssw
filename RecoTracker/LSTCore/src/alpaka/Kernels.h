      ...
      for (unsigned int lowmod : cms::alpakatools::uniform_elements_z(acc, modules.nLowerModules())) {
        ...
        for (unsigned int ix1 : cms::alpakatools::uniform_elements_y(acc, nQuintuplets_lowmod)) {
          ...
          for (unsigned int jx1 : cms::alpakatools::uniform_elements_x(acc, ix1 + 1, nQuintuplets_lowmod)) {
            ...
            unsigned int nHits_ix = 2 * quintuplets.nLayers()[ix];
            unsigned int nHits_jx = 2 * quintuplets.nLayers()[jx];
            int minNHitsForDup = static_cast<int>(0.6f * (nHits_ix < nHits_jx ? nHits_ix : nHits_jx));
            if (nMatched >= minNHitsForDup) {
              // Tiebreak: longer track wins; otherwise rphisum at high pT, DNN score at low pT.
              unsigned int nL_ix = quintuplets.nLayers()[ix];
              unsigned int nL_jx = quintuplets.nLayers()[jx];
              if (nL_ix > nL_jx) {
                rmQuintupletFromMemory(quintuplets, jx);
              } else if (nL_jx > nL_ix) {
                rmQuintupletFromMemory(quintuplets, ix);
              } else {
                float pt_ix = __H2F(quintuplets.innerRadius()[ix]) * lst::k2Rinv1GeVf * 2;
                float pt_jx = __H2F(quintuplets.innerRadius()[jx]) * lst::k2Rinv1GeVf * 2;
                if (pt_ix > 5.0f || pt_jx > 5.0f) {
                  float s1 = __H2F(quintuplets.score_rphisum()[ix]);
                  float s2 = __H2F(quintuplets.score_rphisum()[jx]);
                  if (s1 >= s2)
                    rmQuintupletFromMemory(quintuplets, ix);
                  else
                    rmQuintupletFromMemory(quintuplets, jx);
                } else {
                  if (dnnScore1 <= quintuplets.dnnScore()[jx])
                    rmQuintupletFromMemory(quintuplets, ix);
                  else
                    rmQuintupletFromMemory(quintuplets, jx);
                }
        ...
        for (unsigned int lowmodIdx2 :
          ...
          for (unsigned int ix1 = 0; ix1 < nQuintuplets_lowmod1; ix1 += 1) {
            ...
            for (unsigned int jx1 = 0; jx1 < nQuintuplets_lowmod2; jx1++) {
              ...
              constexpr float d2_thresh = 0.25f;
              if (((dR2 < 0.001f || nMatched >= minNHitsForDup_T5) && d2 < d2_thresh) || (dR2 < 0.02f && d2 < 0.1f)) {
                float pt_ix = __H2F(quintuplets.innerRadius()[ix]) * lst::k2Rinv1GeVf * 2;
                float pt_jx = __H2F(quintuplets.innerRadius()[jx]) * lst::k2Rinv1GeVf * 2;
                bool highPt = (pt_ix > 5.0f || pt_jx > 5.0f);
                bool ix_loses;
                if (isPT5_jx) {
                  ix_loses = true;
                } else if (isPT5_ix) {
                  ...
                  float s1 = __H2F(quintuplets.score_rphisum()[ix]);
                  float s2 = __H2F(quintuplets.score_rphisum()[jx]);
                  ix_loses = (s1 > s2) || (s1 == s2 && ix < jx);
                } else {
                  float d2s = quintuplets.dnnScore()[jx];
                  ix_loses = (dnnScore1 < d2s) || (dnnScore1 == d2s && ix < jx);
                }
                if (ix_loses)
