...
namespace ALPAKA_ACCELERATOR_NAMESPACE::lst {
  ...
  template <typename TAcc>
  ALPAKA_FN_ACC ALPAKA_FN_INLINE float computeRadiusFromThreeAnchorHitsLeg(
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      for (size_t j = 0; j < 3; j++) {
        ...
        for (size_t k = 0; k < 3; k++) {
          float x3 = x3Vec[k];
          float y3 = y3Vec[k];
          candidateRadius = computeRadiusFromThreeAnchorHitsLeg(acc, x1, y1, x2, y2, x3, y3, g, f);
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE bool runQuintupletDefaultAlgoBBBB(TAcc const& acc,
    ...
    float alpha1GeV_OutLo =
        alpaka::math::asin(acc, alpaka::math::min(acc, rt_OutLo * k2Rinv1GeVf / 0.8f, kSinAlphaMax));
    ...
    float muls2 = thetaMuls2 * 9.f / (0.8f * 0.8f) * 16.f;
    ...
    const float zWindow =
        ...
        (zpitch_InLo + zpitch_OutLo);  //FIXME for 0.8f lower than ~0.8 need to add curv path correction
    ...
    float betaInCut =
        alpaka::math::asin(
            acc, alpaka::math::min(acc, (-rt_InSeg * corrF + drt_tl_axis) * k2Rinv1GeVf / 0.8f, kSinAlphaMax)) +
        (0.02f / drt_InSeg);
    ...
    float betaOutCut =
        alpaka::math::asin(acc, alpaka::math::min(acc, drt_tl_axis * k2Rinv1GeVf / 0.8f, kSinAlphaMax)) +
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE bool runQuintupletDefaultAlgoBBEE(TAcc const& acc,
    ...
    const float muls2 = thetaMuls2 * 9.f / (0.8f * 0.8f) * 16.f;
    ...
    float betaInCut =
        alpaka::math::asin(acc, alpaka::math::min(acc, (-sdIn_dr * corrF + dr) * k2Rinv1GeVf / 0.8f, kSinAlphaMax)) +
        (0.02f / sdIn_d);
    ...
    const float dBetaROut2 = dBetaROut * dBetaROut;
    float betaOutCut = alpaka::math::asin(acc, alpaka::math::min(acc, dr * k2Rinv1GeVf / 0.8f, kSinAlphaMax)) +
                       (0.02f / sdOut_d) + alpaka::math::sqrt(acc, dBetaLum2 + dBetaMuls2);
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE bool runQuintupletDefaultAlgo(TAcc const& acc,
    ...
    bridgeRadius = computeRadiusFromThreeAnchorHitsLeg(acc, x2, y2, x3, y3, x4, y4, g, f);
    ...
    if (innerRadius < 0.95f * 0.8f / (2.f * k2Rinv1GeVf))
      return false;
    ...
    float inference = t5dnn::runInference(acc,
                                          ...
                                          outerRadius,
                                          bridgeRadius);
    tightCutFlag = tightCutFlag and (inference > 0.6177366f);  // T5-in-TC cut
    if (inference <= 0.6177366f)                               // T5-building cut
      return false;
