        ...
        for (unsigned int iOff : cms::alpakatools::uniform_elements_y(acc, nQuints)) {
          ...
          for (unsigned int jx : cms::alpakatools::uniform_elements_x(acc, loop_bound)) {
            ...
            if (jx < pixelQuintuplets.nPixelQuintuplets()) {
              ...
              const float d2_lo = isExtT5 ? 0.03f : 0.1f;
              const float d2_hi = isExtT5 ? 0.3f : 1.0f;
              const int otThresh = isExtT5 ? 6 : 4;

              if ((dR2 < 0.02f && d2 < d2_lo) || (dR2 < 1e-3f && d2 < d2_hi)) {
                ...
                for (int i = 0; i < Params_T5::kHits; ++i) {
                  ...
                  if (hit_i == lst::kTCEmptyHitIdx)
                    continue;
                  for (int j = Params_pLS::kHits; j < Params_pT5::kHits; ++j) {
                    if (pixelQuintuplets.hitIndices()[jx][j] == lst::kTCEmptyHitIdx)
                      continue;
              ...
              for (int i = 0; i < Params_T5::kHits; ++i) {
                ...
                for (int j = Params_pLS::kHits; j < Params_pT3::kHits; ++j) {
                  if (pixelTriplets.hitIndices()[ptidx][j] == lst::kTCEmptyHitIdx)
                    continue;
