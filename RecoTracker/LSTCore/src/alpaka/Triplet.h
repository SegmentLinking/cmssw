...
#ifdef WARNINGS
            const unsigned int rightBound = static_cast<unsigned int>(ranges.tripletModuleIndices()[lowerModule1 + 1]);
            if (tripletIndex >= rightBound) {
              printf(
                  "Triplet module occupancy alert! module triplet starting index  = %d, Pair triplet index = %d, next "
                  "module triplet starting index = %d\n",
