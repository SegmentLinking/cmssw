...
std::vector<int> matchedSimTrkIdxs(std::vector<int> hitidxs,
  ...
  return matchedSimTrkIdxs(hitidxs_, hittypes_, trk_simhit_simTrkIdx, trk_ph2_simHitIdx, trk_pix_simHitIdx, verbose);
}
...
std::vector<int> matchedSimTrkIdxs(std::vector<unsigned int> hitidxs,
  ...
  for (size_t ihit = 0; ihit < to_check_duplicate.size(); ++ihit) {
    ...
    for (auto& simhit_idx : (*simHitIdxs).at(hitidx)) {
      ...
      int simtrk_idx = trk_simhit_simTrkIdx.at(simhit_idx);
      if (verbose) {
...
int getDenomSimTrkType(int isimtrk,
  ...
  if (isimtrk < 0)
    return 0;  // not a sim
  const int& q = trk_sim_q.at(isimtrk);
  if (q == 0)
    return 1;  // sim
  const float& pt = trk_sim_pt.at(isimtrk);
  const float& eta = trk_sim_eta.at(isimtrk);
  if (pt < 1 or std::abs(eta) > 2.4)
    return 2;  // sim and charged
  const int& bunch = trk_sim_bunchCrossing.at(isimtrk);
  const int& event = trk_sim_event.at(isimtrk);
  const int& vtxIdx = trk_sim_parentVtxIdx.at(isimtrk);
  const float& vtx_x = trk_simvtx_x.at(vtxIdx);
  const float& vtx_y = trk_simvtx_y.at(vtxIdx);
  const float& vtx_z = trk_simvtx_z.at(vtxIdx);
...
float drfracSimHitConsistentWithHelix(int isimtrk,
  ...
  float vx = trk_simvtx_x.at(0);
  float vy = trk_simvtx_y.at(0);
  float vz = trk_simvtx_z.at(0);
  float pt = trk_sim_pt.at(isimtrk);
  float eta = trk_sim_eta.at(isimtrk);
  float phi = trk_sim_phi.at(isimtrk);
  int charge = trk_sim_q.at(isimtrk);
...
float drfracSimHitConsistentWithHelix(lst_math::Helix& helix,
                                      ...
                                      std::vector<float> const& trk_simhit_z) {
  // Sim hit vector
  std::vector<float> point = {trk_simhit_x.at(isimhitidx), trk_simhit_y.at(isimhitidx), trk_simhit_z.at(isimhitidx)};
