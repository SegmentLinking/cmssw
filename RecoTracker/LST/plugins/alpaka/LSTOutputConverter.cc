...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  void LSTOutputConverter::produce(edm::StreamID sid, device::Event& iEvent, const device::EventSetup& iSetup) const {
    // Setup
    ...
    for (unsigned int i = 0; i < nTrackCandidates; i++) {
      ...
      unsigned int const nPixelHits =
          lstOutput_view.trackCandidateType()[i] == lst::LSTObjType::T5 ? 0 : recHits.size();
      bool pixelSeedIsTriplet = lstOutput_view.trackCandidateType()[i] == lst::LSTObjType::T5
                                    ? false
                                    : lstOutput_view.hitIndices()[i][2] == lstOutput_view.hitIndices()[i][3];
      ...
      for (unsigned int j = nPixelHits + pixelSeedIsTriplet; j < nHits; j++)
      ...
      if (lstOutput_view.trackCandidateType()[i] != lst::LSTObjType::pLS) {
        ...
        if (includeNonpLSTSs_ || lstOutput_view.trackCandidateType()[i] == lst::LSTObjType::T5) {
          ...
          LogDebug("LSTOutputConverter") << "Created a seed with " << seed.nHits() << " " << ss.detId() << " "
      ...
      if (tsosPair.first.isValid()) {
        ...
        edm::LogInfo("LSTOutputConverter")
            << "Failed to make a candidate initial state. Seed state is " << tsos << " TC cand " << i << " "
            << lstOutput_view.trackCandidateType()[i] << " " << lstOutput_view.pixelSeedIndex()[i] << " first hit "
...
DEFINE_FWK_ALPAKA_MODULE(LSTOutputConverter);
