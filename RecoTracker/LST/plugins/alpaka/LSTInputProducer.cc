...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  void LSTInputProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
    ...
    desc.add<std::vector<edm::InputTag>>("pixelSeeds",
                                         std::vector<edm::InputTag>{edm::InputTag("lstInitialStepSeeds"),
                                                                    edm::InputTag("lstHighPtTripletStepSeeds")});
    ...
    for (auto const& seedToken : seedTokens_) {
      ...
      for (auto const& seed : seeds) {
        ...
        if (tscblValid) {
          ...
          dz = (pos.z() - bsPos.z()) -
               ((pos.x() - bsPos.x()) * mom.x() + (pos.y() - bsPos.y()) * mom.y()) / mom.perp() *
                   (mom.z() / mom.perp());
          ...
          double pt2 = pt * pt;
          double p2 = p * p;
          ptErr = std::sqrt(pt2 * p2 / (q * q) * errMat(0, 0) +
                            2.0 * std::sqrt(p2 * pt2) / q * pz * errMat(0, 1) +
                            pz * pz * errMat(1, 1));
