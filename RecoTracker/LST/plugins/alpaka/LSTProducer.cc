...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  public:
    ...
    void acquire(device::Event const& event, device::EventSetup const& setup) override {
      // Inputs
      auto const& pixelSegmentsDC = event.get(lstPixelSeedInputToken_);
      auto const& hitsDC = event.get(lstPhase2OTHitsInputToken_);
  ...
  private:
    edm::EDGetTokenT<lst::PixelSegmentsDeviceCollection> lstPixelSeedInputToken_;
    edm::EDGetTokenT<lst::HitsDeviceCollection> lstPhase2OTHitsInputToken_;
